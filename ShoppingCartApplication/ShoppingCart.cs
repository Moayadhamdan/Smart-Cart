...
namespace ShoppingCartApplication
{
    public class ShoppingCart
    {
        ...
        {
            return items.Sum(item => item.GetDiscountedPrice());
        }
        ...
        public void Shop(object store)
        {
            ...
            else if (store is ElectronicsStore electronicsStore)
            {
                products = electronicsStore.DisplayProducts();
            }
        ...
        private void SelectProduct(List<Product> products, object store)
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {products[i].Name} - {products[i].Price:C} ({products[i].Category}) - Discount: {products[i].Discount}%");
            }
            ...
            if (productIndex >= 0 && productIndex < products.Count)
            {
                ...
                {
                    electronicsStore.AddToCart(products[productIndex], this);
                }
        ...
        public void ViewCart()
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {items[i].Name} - {items[i].Price:C} ({items[i].Category}) - Discounted Price: {items[i].GetDiscountedPrice():C}");
            }

            Console.WriteLine($"Total Cost: {CalculateTotalCost():C}");

            Console.WriteLine("Do you want to remove an item from your cart? (y/n)");
        ...
        public void ShowRandomProducts()
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {randomProducts[i].Name} - {randomProducts[i].Price:C} ({randomProducts[i].Category}) - Discount: {randomProducts[i].Discount}%");
            }
            Console.WriteLine("0. Back to main menu");
        ...
        public void Checkout()
        {
            ...
            Console.WriteLine($"{"Item",-20} {"Category",-15} {"Price",-10} {"Discount",-10} {"Total",-10}");
            ...
            foreach (var item in items)
            {
                ...
                string itemDiscount = item.Discount.ToString() + "%";
                string itemTotal = item.GetDiscountedPrice().ToString("C");

                Console.WriteLine($"{itemName,-20} {itemCategory,-15} {itemPrice,-10} {itemDiscount,-10} {itemTotal,-10}");
            ...
            Console.WriteLine(new string('-', 65));
            Console.WriteLine("Thank you for shopping at Hamdan Mall");
            Console.WriteLine("--------------------");
        }
    }
}
